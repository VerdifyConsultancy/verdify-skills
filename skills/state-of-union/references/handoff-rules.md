...
4. Candidate issues are ready and no higher-priority gate exists -> `sprint-planning`.
5. An approved sprint is active and needs dispatch, monitoring, issue reconciliation, or gate resolution -> `sprint-orchestrator`.
6. Approved lanes await integration, deployment proof, or outcome acceptance -> `release-verification`.
