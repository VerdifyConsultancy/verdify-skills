...
1. Verify each lane's dispatch D, PR identity, I/E/S chain, distinct worker/critic agent and
   ...
   `critic-gate` for `dev`. A `main` promotion separately requires the latest
...
2. Using a protected-base or atomically installed validator, verify every required lane has a valid implementation/evidence/critic-report chain, live current-head `SUCCESS` for `critic-gate` and every configured required check, clean issue/contract reconciliation, and no unresolved blocker. When the target is `main`, also require the real current-head non-author owner approval; never substitute a workflow review.
