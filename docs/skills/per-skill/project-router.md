...
and GitHub state, names one next skill and one mode, and generates ignored local
views so a session can inspect the fresh decision without turning cache into a
competing authority.
...
| Approved phase artifacts | code-selected schemas for transcript, evidence registry, North Star plan/artifacts, project definition, architecture, module contracts, state of union, repo hygiene; specialized sprint/review/delivery validators | upstream lifecycle |
...
|---|---|---|
| `.agent-workflow/router/route-decision.yaml` | `route-decision.schema.yaml` | ignored local machine view |
| `.agent-workflow/router/route-decision.md` | derived Markdown view | ignored local human view |
...
sequenceDiagram
    ...
    PR->>Repo: generate ignored route-decision views
...
- **CLI:** `bin/verdify route --repo <repository> --write` (also `--json`); authority
  inputs are validated against code-selected schemas, and the generated decision
  is validated against `route-decision.schema.yaml` without being committed — see
