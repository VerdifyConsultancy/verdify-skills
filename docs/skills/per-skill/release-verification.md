...
> Assemble review-ready evidence, record runtime diagnostics, integrate externally approved lane heads, prove the intended revision in an authorized environment, and close the human outcome loop.

## Purpose

Assemble a review-ready evidence packet, record observability diagnostics for
planning/review/release health, integrate externally approved S heads and validate the
...
The **REVIEW + release** stage, after `independent-critic`. A critic outcome is
necessary but not sufficient: every lane needs a valid I→E→S chain, live required
checks, and a distinct admin/maintainer's latest effective `APPROVED` review on S.
Integration waits for atomic packet commit P on the evidence-only controller
branch with an `approve` recommendation (or an explicit gated policy exception).
Deployment uses separately scoped, separately authorized access — never worker
credentials.
...
| Mode | What it does |
|---|---|
| `review-inbox` | Verify each lane PR's D/I/E/S chain, exact S approval, and live checks; commit only the packet as P on `controller/<sprint-id>`; gather preview/telemetry/rollback evidence and route the recommendation. |
...
| `integration` | Fresh integration session: revalidate chains, approvals, checks, and P; merge each lane PR individually in dependency order (merge queue when configured), then run full-system/migration/security validation. |
...
| Input | Source | From |
|---|---|---|
| Lane chain I→E→S, lane closeout, critic report, worker/critic identities | lane review state | `independent-critic` |
| PR/MR identity, base ref, exact final S head, latest external approval | GitHub | GitHub control plane |
...
|---|---|---|
| Packet-only commit P containing `.agent-workflow/sprints/<sprint-id>/review/review-inbox-packet.yaml` on `controller/<sprint-id>` | `review-inbox-packet.schema.yaml` | reviewers, integration gate |
...
sequenceDiagram
    ...
    Critic-->>RV: valid D/I/E/S lane chains + critic outcomes
    RV->>GH: verify each S, external approval, checks, mergeability
    RV->>RV: commit packet alone as P on evidence-only controller branch
    RV->>GH: merge each lane PR individually in dependency order
...
- **Review packet completeness** — `evidence_completeness.verdict` is `complete`
  only with PR/MR identity, exact S head per lane, recorded external reviewer
  identity and latest `APPROVED` review, observed check statuses,
  ...
  unresolved preview-generator/secret controls. The packet must be the only path
  in commit P. The controller branch is evidence-only, never the integration
  candidate; after P only canonical release, outcome, status, or an atomic
  sprint-plan/status terminalization commit may follow.
- **Lane integration is individual** — merge or queue each lane PR against its
  approved base; never merge the controller evidence branch.
...
- **Upstream:** `independent-critic` (per-lane critic outcome), the external
  admin/maintainer approval on S, and `sprint-orchestrator`
