...
Verdify exposes eight skills but preserves the detailed lifecycle as explicit modes. A skill may advance through its own modes without reactivation, provided each mode's artifacts and gates are satisfied.

## 1. Project router

Inspects Git, GitHub state or snapshot, and `.verdify` artifacts. It writes a route decision and names exactly one next skill/mode. It does not manufacture missing project facts.
