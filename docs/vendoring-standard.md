...
3. **An exact manifest verification step, per vendor.** Release archives ship
   `MANIFEST.sha256`, and `scripts/verify-package.sh` requires exact typed
   archive membership before extraction. It rejects missing, changed, extra,
   duplicate, unsafe, and wrong-type members, then verifies every regular-file
   digest and discovery-symlink target. Vendors record that verification in
   `UPSTREAM.yaml`.
...
The npm installer enforces the same transition: a successful install removes
older managed version directories after new links and manifests are complete;
a failed pack install removes the new package copy and preserves the prior
version and operator-owned conflicts.
...
- **#120** — release 1.3.0 carries the exact-tarball, exact-ZIP, and resumable
  transaction gates described above; fleet fan-out waits for that release.
