...
- Added the reusable `workflow_call` CI workflow `.github/workflows/compliance-gate.yml` (npx- or vendored-sourced, `strict`/`report_only` inputs) and the `compliance-assessment.schema.yaml` schema, so any fleet repo can wire the compliance gate as a check. Note the context a
  caller sees is `<caller-job-id> / compliance`, not `verdify-compliance` —
  in this repo `compliance-selftest.yml`'s `compliance` job calls it and reports
  as `compliance / compliance`. `verdify-compliance` is the reusable workflow's
  display name and will never appear as a status context. Upstream of jvallery/agents#2026 enforcement.
